...
>    show (Register ts) = intercalate ";" (show <$> ts)

> -- intercalate :: [a] -> [[a]] -> [a]
> -- exemplo: intercalate "+" ["a", "b", "c"] = "a+b+c"
...
> checkHour x 
>       | x >= 0 && x <= 23 = Success x
>       | otherwise = Failure [InvalidHour x]
...
> checkMinute x
>       | x >= 0 && x <= 60 = Success x
>       | otherwise = Failure [InvalidMinute x]
...
> checkTime (h, m)
>        | checkHour h == Success h && checkMinute m == Success m = Success (Time h m)
>        | checkHour h == Failure [InvalidHour h] && checkMinute m == Success m = Failure [InvalidHour h]
>        | checkHour h == Success h && checkMinute m == Failure [InvalidMinute m] = Failure [InvalidMinute m]
>        | otherwise = Failure [InvalidHour h, InvalidMinute m]
